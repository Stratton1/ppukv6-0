...
const PhotoGallery = ({ propertyId, canUpload = false }: PhotoGalleryProps) => {
  ...
  const fetchPhotos = async () => {
    try {
      // First attempt: try to use type column if it exists
      let { data, error } = await supabase
        .from("media")
        .select("id, property_id, url, caption, room_type, created_at, type")
        .eq("property_id", propertyId)
        .eq("type", "photo")
        .order("created_at", { ascending: false });

      // If type column doesn't exist, fallback to mime_type filter
