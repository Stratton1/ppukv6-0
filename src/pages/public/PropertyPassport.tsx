...
import { DocumentUploader, PhotoGallery, APIPreviewCard, PassportScore } from "@/components";
  ...
  const fetchProperty = async () => {
    try {
      ...
      if (id) {
        ...
        // No property ID, but user is logged in - try to find their property
        const { data: userProperties, error: queryError } = await supabase
          .from("properties")
          .select("*")
        ...
        if (userProperties && userProperties.length > 0) {
          propertyData = userProperties[0];
        } else {
          // No property found for user
          setProperty(null);
          setIsOwner(false);
          setLoading(false);
          return;
