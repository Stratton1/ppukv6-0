 ...
 */
async function fetchByTitleNumber(titleNumber: string, apiClient: ExternalApiClient, logger: Logger): Promise<HMLRData> {
  try {
    ...
    const hmlrData: HMLRData = {
      ...
      transactions: title.transactions ? title.transactions.map((t: any) => ({
      ...
      charges: title.charges ? title.charges.map((c: any) => ({
      ...
      restrictions: title.restrictions ? title.restrictions.map((r: any) => ({
 ...
 */
async function searchByPostcode(request: any, apiClient: ExternalApiClient, logger: Logger): Promise<HMLRData> {
  try {
