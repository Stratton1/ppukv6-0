...
const EPC_API_BASE_URL =
  Deno.env.get("EPC_API_BASE_URL") || "https://epc.opendatacommunities.org/api/v1";
...
async function fetchEPCData(request: any, logger: Logger): Promise<EPCData> {
  const apiClient = new ExternalApiClient(EPC_API_BASE_URL, EPC_API_KEY, logger);
  ...
  try {
    ...
    const epcData: EPCData = {
      ...
      environmentalImpactEfficiency:
        parseInt(latestCertificate["environmental-impact-efficiency"]) || 0,
      ...
      lowEnergyFixedLightingOutletsCount:
        parseInt(latestCertificate["low-energy-fixed-lighting-outlets-count"]) || 0,
