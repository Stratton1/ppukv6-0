...
async function testA1_ViewPhotos() {
  const { data, error } = await supabase.from("property_photos").select("*").limit(1);
...
async function testA3_FileSizeValidation() {
  ...
  const { data, error } = await supabase.from("property_photos").select("file_name").limit(1);
...
async function testB2_UploadDocument() {
  ...
  const { data, error } = await supabase
    .from("documents")
    .select(
      "id, property_id, document_type, file_name, file_size_bytes, mime_type, description, uploaded_by"
    )
    .limit(1);
...
async function testB4_DocumentTypeValidation() {
  ...
  const { data, error } = await supabase.from("documents").select("document_type").limit(1);
...
async function testB5_DocSizeLimit() {
  ...
  const { data, error } = await supabase.from("documents").select("file_size_bytes").limit(1);
