  ...
  try {
    ...
    if (!mediaError && recentMedia && recentMedia.length > 0) {
      ...
      recentMedia.forEach((item, index) => {
        console.log(
          `    ${index + 1}. ${item.type} - ${item.caption || "No caption"} (${new Date(item.created_at).toLocaleDateString()})`
        );
      });
    }
  } catch (error) {
    ...
    if (!docsError && recentDocs && recentDocs.length > 0) {
      ...
      recentDocs.forEach((doc, index) => {
        const size = doc.file_size_bytes
          ...
          : "Unknown size";
        console.log(
          `    ${index + 1}. ${doc.file_name} (${size}) - ${new Date(doc.created_at).toLocaleDateString()}`
        );
      });
    }
  } catch (error) {
...
function generateReport() {
  ...
  console.log(
    `  property-photos: ${results.buckets["property-photos"].exists ? "✅" : "❌"} (${results.buckets["property-photos"].public ? "Public" : "Private"})`
  );
  console.log(
    `  property-documents: ${results.buckets["property-documents"].exists ? "✅" : "❌"} (${results.buckets["property-documents"].public ? "Public" : "Private"})`
  );
