...
test.describe("Validation and Security", () => {
  ...
  test("Property passport completion score updates correctly", async ({ page }) => {
    ...
    // Upload a document
    await uploadHelpers.uploadDocument(TEST_FILES.smallDocument.name, "epc", "Test EPC document");
  ...
  test("Signed URL generation works for document downloads", async ({ page }) => {
    ...
    if (docCount > 0) {
      ...
      const downloadPromise = page.waitForResponse(
        response => response.url().includes("createSignedUrl") && response.status() === 200
      );
